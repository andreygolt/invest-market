...
export default function SubmitPage() {
  ...
  if (loading) {
    return <div className="min-h-screen flex items-center justify-center bg-slate-50"><p className="text-slate-500">Загрузка...</p></div>;
  }

  if (!project) {
    return <div className="min-h-screen flex items-center justify-center bg-slate-50"><p className="text-slate-500">Проект не найден</p></div>;
  }

  const isDraft = project.status === 'draft';
  const statusLabel = STATUS_LABELS[project.status] ?? project.status;
  const statusColor = STATUS_COLORS[project.status] ?? 'bg-slate-100 text-slate-700';
    ...
    <main className="min-h-screen bg-slate-50 py-10 px-4">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-6">
          <h1 className="text-xl font-semibold text-slate-900 mb-4">Статус заявки</h1>
        ...
        {/* Видео-питч */}
        <div className="rounded-xl border border-slate-200 bg-white p-6">
          <h2 className="text-lg font-medium text-slate-900 mb-2">Видео-питч</h2>
          ...
          {project.video_path ? (
            <div className="flex items-center gap-3 p-3 rounded-xl border border-emerald-200 bg-emerald-50">
              ...
              <span className="text-sm text-emerald-700 flex-1 truncate">Видео загружено</span>
              {isDraft && (
                <button onClick={deleteVideo} className="text-red-600 text-xs hover:text-red-700 shrink-0">
                  Удалить
                </button>
              )}
            </div>
            ...
            <div>
              ...
              <button
                ...
                className="w-full text-center px-4 py-4 border-2 border-dashed border-slate-300 rounded-xl text-sm text-slate-600 hover:border-slate-400 hover:text-slate-900 disabled:opacity-50"
              >
        ...
        {isDraft && (
          <div className="rounded-xl border border-slate-200 bg-white p-6">
            <h2 className="text-lg font-medium text-slate-900 mb-2">Отправить на модерацию</h2>
            <p className="text-sm text-slate-500 mb-4">
              После отправки редактирование анкеты будет недоступно. Убедитесь, что все данные заполнены корректно.
            </p>
            <p className="text-xs text-slate-600 rounded-xl border border-slate-200 bg-slate-50 p-3 mb-4">
            ...
            <button
              ...
              disabled={submitting}
              className="w-full py-3 bg-slate-900 text-white rounded-xl text-sm font-medium hover:bg-slate-700 disabled:opacity-50"
            >
