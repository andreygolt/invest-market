# T94 — Светлая тема внутри приложения + горизонтальный фильтр каталога

## Контекст

T92 и T93 не были выполнены (usage limit). Повтор задачи.
...
## Часть 2: Каталог — светлая тема + горизонтальный фильтр
...
3. `/catalog` — светлый фон, горизонтальные фильтры, светлые карточки
4. `/dashboard` — светлый фон
5. `/deals/[id]` — светлый фон
6. Лендинг `/` — остаётся тёмным
7. Записать в progress.md: `DONE: T94` + что создано/изменено
