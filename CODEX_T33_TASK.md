# T33 — Система платформенных уведомлений

## Контекст

T28, T29, T30, T31, T32 не были выполнены (Codex hit usage limit). T33 реализует ту же задачу.
...
T33 добавляет систему in-app уведомлений:
...
3. `npm test` — все тесты проходят (включая новые t33.test.ts)
...
9. Записать в `progress.md`: `DONE: T33 + что создано/изменено`
