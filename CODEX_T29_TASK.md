# T29 — Система платформенных уведомлений

## Контекст

T28 не была выполнена (Codex hit usage limit). T29 реализует ту же задачу.
...
T29 добавляет систему in-app уведомлений:
...
3. `npm test` — все тесты проходят (включая новые t29.test.ts)
...
9. Записать в `progress.md`: `DONE: T29 + что создано/изменено`
