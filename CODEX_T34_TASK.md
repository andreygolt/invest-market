# T34 — Система платформенных уведомлений

## Контекст

T28, T29, T30, T31, T32, T33 не были выполнены (Codex hit usage limit). T34 реализует ту же задачу.
...
T34 добавляет систему in-app уведомлений:
...
3. `npm test` — все тесты проходят (включая новые t34.test.ts)
...
9. Записать в `progress.md`: `DONE: T34 + что создано/изменено`
