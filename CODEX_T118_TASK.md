# T118 — Тесты для API избранного инвестора

## Цель

Четыре маршрута избранного инвестора существуют без тестового покрытия:

- `GET /api/investor/favorites` — список избранного с данными проектов
- `POST /api/investor/favorites` — добавить/обновить запись (upsert)
- `PATCH /api/investor/favorites/[id]` — обновить notes/personal_status
- `DELETE /api/investor/favorites/[id]` — удалить из избранного

Нужно создать `__tests__/t118.test.ts` с тестами для всех четырёх маршрутов.
...
- `app/api/investor/favorites/route.ts` — GET + POST, используют `createAdminClient`
- `app/api/investor/favorites/[id]/route.ts` — PATCH + DELETE, используют `createAdminClient`

GET запрашивает `investor_favorites` с join `projects(name)`, затем дополнительно `v_investor_catalog` для industry/stage/ai_score.

POST выполняет upsert в `investor_favorites` с проверкой наличия approved-проекта.

PATCH находит запись, проверяет owner, обновляет поля.

DELETE находит запись, проверяет owner, удаляет.

---

## Создать `__tests__/t118.test.ts`

```typescript
// __tests__/t118.test.ts
...
describe('T118 PATCH /api/investor/favorites/[id]', () => {
  ...
  async function loadPatchRoute(options?: {
    ...
    const updateSelectEq = jest.fn(() => ({ single: updateSingle }));
    const updateSelectMock = jest.fn(() => ({ eq: updateSelectEq }));
...
- НЕ трогать `app/api/investor/favorites/route.ts` — только тестируем
- НЕ трогать `app/api/investor/favorites/[id]/route.ts` — только тестируем
...
| Файл | Действие |
|------|----------|
| `__tests__/t118.test.ts` | СОЗДАТЬ — тесты для четырёх маршрутов избранного |
...
3. `npm test` — новые тесты проходят (минимум 18 тестов в t118)
4. Существующие тесты (~160 тестов) не сломаны
5. Записать в progress.md: `DONE: T118` + что создано
...
```
DONE: T118
- создан __tests__/t118.test.ts: 18 тестов для GET /api/investor/favorites, POST /api/investor/favorites, PATCH /api/investor/favorites/[id], DELETE /api/investor/favorites/[id]
```
