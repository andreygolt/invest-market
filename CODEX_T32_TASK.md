# T32 — Система платформенных уведомлений

## Контекст

T28, T29, T30, T31 не были выполнены (Codex hit usage limit). T32 реализует ту же задачу.
...
T32 добавляет систему in-app уведомлений:
...
3. `npm test` — все тесты проходят (включая новые t32.test.ts)
...
9. Записать в `progress.md`: `DONE: T32 + что создано/изменено`
