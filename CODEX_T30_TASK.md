# T30 — Система платформенных уведомлений

## Контекст

T28 и T29 не были выполнены (Codex hit usage limit). T30 реализует ту же задачу.
...
T30 добавляет систему in-app уведомлений:
...
3. `npm test` — все тесты проходят (включая новые t30.test.ts)
...
9. Записать в `progress.md`: `DONE: T30 + что создано/изменено`
