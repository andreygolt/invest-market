# T28 — Система платформенных уведомлений
...
T28 добавляет систему in-app уведомлений:
...
1. Получить `investor_id` и `project_id` из заявки (уже есть в `select: 'id, status'` — добавить `investor_id`)
...
- Список уведомлений (shadcn/ui `ScrollArea`, max-height: 400px):
  - Каждое уведомление: `Card` или `div` с:
    - `title` (полужирный если непрочитано)
    - `body` (серый текст)
    - `created_at` → `toLocaleDateString('ru-RU')`
    - Кнопка «✕» — PATCH `/api/notifications/[id]/read`
...
**Иконка:** Используй HTML-сущность `🔔` или символ `⚑` — без новых svg-пакетов.
Или простую кнопку с текстом «[N]» если emoji мешают линтеру.
...
- shadcn/ui компоненты (Popover, ScrollArea, Badge, Button, Card)
...
3. `npm test` — все тесты проходят (включая новые t28.test.ts)
...
9. Записать в `progress.md`: `DONE: T28 + что создано/изменено`
