# T70 — Кабинет проекта: таймлайн изменений статуса

## Контекст

T67, T68 и T69 не были выполнены Codex (hit usage limit). T70 повторяет задачу T67/T68/T69 без изменений.
...
Доступ: только роль `project` (владелец проекта).
...
### 5. Создать `__tests__/t70.test.ts`
...
3. `npm test` — все тесты проходят (включая новые t70.test.ts)
...
8. Записать в `progress.md`: `DONE: T70 + что создано/изменено`
