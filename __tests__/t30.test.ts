...
describe('T30 notifications', () => {
  ...
  it('GET /api/notifications returns notifications and unread_count', async () => {
    ...
    const response = await GET_NOTIFICATIONS(new NextRequest('http://localhost/api/notifications'));
    const body = (await response.json()) as { notifications: NotificationRow[]; unread_count: number };
  ...
  it('PATCH /api/admin/applications/[id] status=approved returns 200 when createNotification fails', async () => {
